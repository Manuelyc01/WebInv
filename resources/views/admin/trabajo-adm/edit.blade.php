...
@section('content')
    @if(isset($element))
        {!! Form::model($element, [ 'route' => ['trabajo-adm.update', $element->id], 'method' => 'PUT', 'id' => 'admin-form' ]) !!}
    @else
        {!! Form::open(['route' => 'trabajo-adm.store', 'method' => 'POST' , 'id' => 'admin-form']) !!}
    @endif

    @include('admin.trabajo-adm.forms.form')
    {!! Form::close() !!}

    @isset($element)
        {{ Form::open(['route' => ['trabajo-adm.destroy' , $element->id] , 'method' => 'DELETE', 'id' => 'form-delete-detail']) }}
        {{ Form::close() }}
    @endisset
    ...
    <div class="col-md-12">
        ...
        @endisset
        <a class="btn btn-info btn-addon m-b-sm" href="{{ route('trabajo-adm.index') }}">
