...
@section('content')
    @if(isset($element))
        {!! Form::model($element, [ 'route' => ['servicio-adm.update', $element->id], 'method' => 'PUT', 'id' => 'admin-form' ]) !!}
    @else
        {!! Form::open(['route' => 'servicio-adm.store', 'method' => 'POST' , 'id' => 'admin-form']) !!}
    @endif

    @include('admin.servicio-adm.forms.form')
    {!! Form::close() !!}

    @isset($element)
        {{ Form::open(['route' => ['servicio-adm.destroy' , $element->id] , 'method' => 'DELETE', 'id' => 'form-delete-detail']) }}
        {{ Form::close() }}
    @endisset
    ...
    <div class="col-md-12">
        ...
        @endisset
        <a class="btn btn-info btn-addon m-b-sm" href="{{ route('servicio-adm.index') }}">
