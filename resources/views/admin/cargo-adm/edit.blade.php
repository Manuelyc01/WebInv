...
@section('content')
    @if(isset($element))
        {!! Form::model($element, [ 'route' => ['cargo-adm.update', $element->id], 'method' => 'PUT', 'id' => 'admin-form' ]) !!}
    @else
        {!! Form::open(['route' => 'cargo-adm.store', 'method' => 'POST' , 'id' => 'admin-form']) !!}
    @endif

    @include('admin.cargo-adm.forms.form')
    {!! Form::close() !!}

    @isset($element)
        {{ Form::open(['route' => ['cargo-adm.destroy' , $element->id] , 'method' => 'DELETE', 'id' => 'form-delete-detail']) }}
        {{ Form::close() }}
    @endisset
    ...
    <div class="col-md-12">
        ...
        @endisset
        <a class="btn btn-info btn-addon m-b-sm" href="{{ route('cargo-adm.index') }}">
