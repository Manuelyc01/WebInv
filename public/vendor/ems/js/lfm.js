(function( $ ){
  ...
  $.fn.filemanager = function(type, options) {
    ...
    this.on('click', function(e) {
      var route_prefix = (options && options.prefix) ? options.prefix : '/es/laravel-filemanager';
    ...
    $(document).on('click', '.fm-button' , function(e){
        localStorage.setItem('target_input', $(this).data('input'));
        localStorage.setItem('target_preview', $(this).data('preview'));
        window.open('/es/laravel-filemanager?type=' + type, 'FileManager', 'titlebar=no,toolbar=no,location=no,status=no,menubar=no,width=900,height=600');
        return false;
    })
  }
