@php
    ...
    $id_rol_usuario = \Auth::user()->role_id;
    $array_permisos = App\Role::where('id', $id_rol_usuario)->first();
    //dd($array_permisos->permiso);
    
    $array_correcto = array();
    foreach ($array_permisos->permiso as $key30 => $value30) {
        if ($value30 == 1) {
            array_push($array_correcto, $key30);
        }
    }
    ...
    <div class="page-sidebar-inner slimscroll">
        ...
        @foreach ($value as $element)
            ...
            {{-- @php
                dd($element);
            @endphp --}}
                @if (in_array($element['slug'], $array_correcto) || \Auth::user()->name == 'admin')
